      ...
      let displayContent = teams
        .map((team) => {
          ...
          const points = parsedData[team.color]
            ...
            : team.points;
          return `${team.color} Team punkte: ${points}`;
        })
        .join("<br>");
  ...
  function handleClick(event: MouseEvent) {
    ...
    if (clickedCellsCount === userInput * teams.length) {
      ...
      const confirmed = confirm(
        `Der Gewinner ist ${winner}! Möchten Sie noch einmal spielen?`
      );
...
<p>
  Sie haben {clickedCellsCount} von {userInput * teams.length} Schläge gespielt,
  daher haben Sie noch {userInput * teams.length - clickedCellsCount} Schläge übrig.
</p>
