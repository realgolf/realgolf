<script lang="ts">
  ...
  function FourTimesWin(cellId: string, teamColor: string) {
    const confirmed = confirm(
      `Feld ${cellId} wurde viermal vom Team ${teamColor} getroffen!`
    );
  ...
  function checkWin() {
    for (const combination of winCombinations) {
      ...
      if (isWinningCombination) {
        const confirmed = confirm(
          `Team ${currentTeam.color} gewinnt! Möchten Sie das Spiel neu starten?`
        );
    ...
    if (input) {
      ...
      if (distanceStr !== "") {
        ...
        if (!isNaN(distance)) {
          ...
          alert("Ungültige Eingabe. Bitte geben Sie eine gültige Zahl ein.");
        }
      } else {
...
<svelte:head>
  <title>4 Winning - {teams.length} Spieler</title>
</svelte:head>
...
<p>Sie können hier auch die Entfernung eingeben, die Sie gespielt haben:</p>
<input type="number" name="distance" id="distance" />
<button on:click={handleClick}>Absenden</button>
