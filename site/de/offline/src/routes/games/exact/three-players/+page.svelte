<script lang="ts">
	...
	let clickedCellsCount: number = 0;
	...
	function updatePointsDisplay() {
		...
		if (display) {
			let displayContent = teams
				.map((team) => `${team.color} Team Punkte: ${team.points}`)
				.join('<br>');
	...
	function handleClick(event: MouseEvent) {
		...
		if (match) {
			...
			if (clickedCell) {
				...
				if (team) {
					...
					lastRowNumbers[color] = rowNumber;
		...
		if (clickedCellsCount === userInput * teams.length) {
			...
			const confirmed = confirm(`Der Gewinner ist ${winner}! Möchten Sie noch einmal spielen?`);
	...
	function updateTeamTurn() {
		...
		if (teamTurnDisplay) {
			teamTurnDisplay.innerHTML = `Aktuelles Team am Zug: ${color}`;
		}
	}
...
<svelte:head>
	<title>Exact - {teams.length} Spieler</title>
</svelte:head>
...
<p>
	Geben Sie die Anzahl der Schläge pro Team ein: <input
...
<p>
	Sie haben {clickedCellsCount} von {userInput * teams.length} Schlägen gespielt, daher haben Sie noch
	{userInput * teams.length - clickedCellsCount} Schläge übrig.
</p>
