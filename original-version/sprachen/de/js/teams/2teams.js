  ...
  const checkAndAnnounceWin = (cells) => {
    for (let i = 0; i <= cells.length - 4; i++) {
      ...
      if (windowSlice.every((cell) => cell === team0)) {
        console.log("Yea!!");
        writeInfo(
          "Das Dunkelgoldene Team hat das Spiel gewonnen. <br> Herzlichen Glückwunsch!"
        );
        changeTeam();
        currentPlayer();
        setTimeout(() => {
  ...
  const checkAndAnnounceWin = (cells) => {
    for (let i = 0; i <= cells.length - 4; i++) {
      ...
      if (windowSlice.every((cell) => cell === team1)) {
        writeInfo(
          `Das Dunkelrote Team hat das Spiel gewonnen. <br> Herzlichen Glückwunsch!`
        );
        setTimeout(() => {
          hideInfo();
          setNull();
...
function currentPlayer() {
  if (teams == 1) {
    ...
    document.getElementById("currentPlayer").innerHTML =
      "Team: Das Dunkelgoldene Team ist dran!";
  } else if (teams == 0) {
    document.getElementById("currentPlayer").style.backgroundColor = team1;
    document.getElementById("currentPlayer").innerHTML =
      "Team: Das Dunkelrote Team ist am Zug!";
  } else {
    console.log(
      "Bitte Kontaktieren sie unseren Developer um im das Problem mit zu teilen! E-Mail: moinjulian.dev@outlook.com"
    );
  }
}
